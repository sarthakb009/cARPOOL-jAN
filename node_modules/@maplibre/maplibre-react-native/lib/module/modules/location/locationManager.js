...
export const LocationModuleEventEmitter = new NativeEventEmitter(MapLibreGLLocationManager);
...
class LocationManager {
  ...
  async getLastKnownLocation() {
    if (!this._lastKnownLocation) {
      ...
      try {
        lastKnownLocation = await MapLibreGLLocationManager.getLastKnownLocation();
      } catch (error) {
  ...
  start(displacement = 0) {
    if (!this._isListening) {
      ...
      this.subscription = LocationModuleEventEmitter.addListener(MapLibreGL.LocationCallbackName.Update, this.onUpdate);
