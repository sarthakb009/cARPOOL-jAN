...
import { Box, Text, VStack, HStack, Icon, Button, Pressable, Modal, Progress, useDisclose, Select, FlatList, Switch, useToast } from 'native-base';
...
const RecurringRidesScreen = () => {
  ...
  const fetchUserDetails = useCallback(async () => {
    try {
      ...
      // Assuming roles can be an array
      if (Array.isArray(response.data.roles)) {
        setIsDriver(response.data.roles.includes("ROLE_DRIVER"));
      } else {
  ...
  const handleLocationSelect = useCallback((isPickup) => {
    navigation.navigate('SelectLocation', {
      isPickup,
      onSelect: (location) => {
        // Assuming location is a string address
        if (isPickup) {
          setPickup(location || '');
        } else {
          setDropoff(location || '');
        }
        if (isPickup && location && dropoff) {
          setCurrentStep(1);
        }
        if (!isPickup && location && pickup) {
          setCurrentStep(1);
        }
      },
    });
  ...
  const handleScheduleRecurringRide = useCallback(async () => {
  ...
  }, [pickup, dropoff, startDate, endDate, time, frequency, userDetails, isDriver, navigation]);
            ...
            <Box bg="white" p={4} borderRadius="2xl" shadow={3}>
              <VStack space={2}>
                <HStack justifyContent="space-between">
                  ...
                  <Text color="#333">{pickup || 'N/A'}</Text>
                </HStack>
                <HStack justifyContent="space-between">
                  <Text fontWeight="bold" color="#333">Drop-off:</Text>
                  <Text color="#333">{dropoff || 'N/A'}</Text>
                </HStack>
                <HStack justifyContent="space-between">
                  <Text fontWeight="bold" color="#333">Start Date:</Text>
                  <Text color="#333">{startDate ? format(startDate, 'MMM dd, yyyy') : 'N/A'}</Text>
                </HStack>
                <HStack justifyContent="space-between">
                  <Text fontWeight="bold" color="#333">End Date:</Text>
                  <Text color="#333">{endDate ? format(endDate, 'MMM dd, yyyy') : 'N/A'}</Text>
                </HStack>
                <HStack justifyContent="space-between">
                  <Text fontWeight="bold" color="#333">Time:</Text>
                  <Text color="#333">{time ? format(time, 'hh:mm a') : 'N/A'}</Text>
                </HStack>
                <HStack justifyContent="space-between">
                  <Text fontWeight="bold" color="#333">Frequency:</Text>
                  <Text color="#333">{frequency || 'N/A'}</Text>
                </HStack>
                <HStack justifyContent="space-between">
  ...
  }, [currentStep, fadeAnim, slideAnim, scaleAnim, handleLocationSelect, pickup, dropoff, startDate, endDate, time, frequency, isDriver, onTimePickerOpen]);
    ...
    <ScrollView contentContainerStyle={styles.container}>
      <VStack space={6}>
        ...
        <Animated.View style={{ width: progressAnim.interpolate({ inputRange: [0, 100], outputRange: ['0%', '100%'] }) }}>
          <Progress value={(currentStep + 1) * (100 / STEPS.length)} colorScheme="blue" size="xs" />
        </Animated.View>
            ...
            <Animated.View style={{ opacity: fadeAnim, transform: [{ translateX: Animated.multiply(slideAnim, -1) }] }}>
              <Button
                onPress={() => {
                  ...
                  if (currentStep === 0 && (!pickup || !dropoff)) {
                    Alert.alert('Error', 'Please select both pickup and drop-off locations.');
                    return;
                  }
                  if (currentStep === 1 && (!startDate || !endDate || !time || !frequency)) {
                    Alert.alert('Error', 'Please fill in all scheduling details.');
                    return;
                  }
        ...
        <Modal.Content maxWidth="400px">
          ...
          <Modal.Body>
            <Calendar
              ...
              markedDates={{
                [format(startDate, 'yyyy-MM-dd')]: { selected: true, selectedColor: '#4299e1' },
                [format(endDate, 'yyyy-MM-dd')]: { selected: true, selectedColor: '#4299e1' }
              }}
              theme={{
